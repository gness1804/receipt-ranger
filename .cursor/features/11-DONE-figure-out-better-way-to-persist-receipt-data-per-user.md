...
Right now, we have the file processed_receipts.json which contains a record of receipts that were run. This worked fine in a localhost application when it was just me running receipts. But if multiple users use my application, this file could get crowded, and it could be hard to distinguish between users. I think we have a few options:
...
We will go with either number one or number two. We need to decide on which path to take, or if another path that I didn't list makes more sense, as a solution to the problem of persisting receipt data per user. 
